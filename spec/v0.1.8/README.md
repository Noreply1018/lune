...
状态：规划中。本目录沉淀 v0.1.8 的发布范围、路由策略设计、quota 文案分层、卡片展示稳定性和验收矩阵；所有写入本目录的事项均视为 v0.1.8 必须完成并验收的范围。

本版本首个目标是把 Pool 内账号调度从隐式“健康优先”升级为显式 Pool 级路由策略，让用户可以在“优先成功率”和“严格尊重账号排序”之间做选择。该能力不改变账号硬阻断规则，不允许绕过需要重登、额度明确阻断、订阅异常、runtime binding 不可用或服务冷却等不可接普通流量状态。
...
1. 默认策略必须延续现有“健康优先”的调度意图；v0.1.8 同时修正模型兜底边界，明确不支持请求模型的账号不得被兜底选中。
2. Pool 中的账号拖拽顺序需要有清晰语义：健康优先下是同健康层级内的顺序；排序优先下是主要调度顺序。
3. “排序优先”只影响可接普通流量账号之间的选择顺序，不绕过硬阻断状态。
4. 降级需要分层：轻降级可以继续接流量；硬阻断必须跳过。
5. 路由结果必须可解释。用户把账号排第一却没有被选中时，UI / 日志需要能说明跳过原因。
6. 重试必须沿用当前 Pool 的路由策略，并排除已尝试失败的账号。
7. UI 控件要克制：在现有“自检 Pool”右侧增加策略切换按钮，不新增大段说明或新的复杂面板。
8. Quota 快照、quota 辅助接口失败和真实模型请求限流必须分层展示；前端不能只根据 `cpa_quota_status='error'` 推断“模型请求被限流”。
9. Pool 账号卡片 chip 是摘要层，不要求完整铺开所有文字；优先保证网格高度稳定和扫描效率。
10. 卡片 chip 行必须稳定为单行摘要；完整解释应放在 title、tooltip、详情抽屉或诊断区。
11. 解决 chip 高度异常必须约束布局根因，不能只依赖缩短某一个当前文案。
...
- `ordered` 不允许绕过硬阻断：禁用、member 禁用、凭据不可用、runtime binding 不可用、订阅非 active、quota blocked、Codex 模型请求 429 阻断证据、serving cooldown 未过期、serving error、明确不支持模型都必须跳过。
...
- v0.1.7 中 `今日 N / N 天后到期 / 模型请求被限流` 三 chip 组合会触发卡片高度不一致，v0.1.8 必须修复。
- 卡片 chip 行采用单行、不换行、溢出省略的摘要口径。
- `模型请求被限流` 可以在卡片层缩短为 `请求限流` 或 `模型限流`；详情页和诊断区保留完整解释。

## 版本边界

v0.1.8 当前范围聚焦 Pool 内账号选择策略、Codex CPA quota 诊断文案分层，以及 Pool 账号卡片展示稳定性。不新增全局多 Pool 调度，不新增权重随机，不新增按成本、额度百分比或每日配额消耗的复杂调度算法；quota 文案修正也不新增更细数据库状态枚举，优先在现有字段基础上分层解释；chip 高度问题不新增后端状态字段，也不改变 Codex `429` 的后端归类规则。
