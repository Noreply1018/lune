...
状态：已发布。本目录已经沉淀 v0.1.7 必须闭环项、测试矩阵和本地/容器验证证据；最终 `gpt-5.5` subagent 严格审计、发布阻塞提交、`v0.1.7` tag 和公开镜像发布均已完成。

本目录按“问题闭环”组织 v0.1.7 规格。每份文档围绕一个真实 UI / 运行态问题展开，统一描述问题、改进策略、UI 表现、日志与诊断、测试与验收、已决策记录或审计记录，避免详情抽屉、诊断页和交互规则散落在不同文件后互相遗漏。
...
v0.1.7 作为 UI 与诊断体验版本，收敛账号详情抽屉、直连账号诊断页、Pool token 编辑、Codex 429 状态归类、最小 provider pinning 状态数据源和 Add Account auth JSON 单文件导入。本目录只保留 v0.1.7 已发布范围和验收事实。
