...
状态：验收矩阵已确认。本轮已完成 CPA 删除后重导入核心复现、生命周期审计补强、真实请求诊断 evidence 回写、数据保留清理审计、隔离 `stateful-probe` 证据采集、`c` 额度不足真实验证和 quota 鉴权失败但可用真实验证；`al` 封号真实样本尚未完成，v0.2.0 仍未达到严格发布完成标准。
