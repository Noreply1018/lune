...
- **CPA 管理** — 复用 CPA OAuth 登录、凭据热加载、远程账号批量导入、过期预警
...
- 默认 Compose 会启用 CPA management API，Lune 的账号登录会直接复用 CPA 已有的 OAuth 管理能力
...
| CPA Login | `POST /accounts/cpa/login-sessions`, `GET /accounts/cpa/login-sessions/{id}`, `POST /accounts/cpa/login-sessions/{id}/callback`, `POST /accounts/cpa/login-sessions/{id}/cancel` |
